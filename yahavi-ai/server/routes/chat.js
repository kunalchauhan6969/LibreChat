...
const OPENAI_COMPAT = {
  ...
  openrouter:   {
    ...
    headers: { 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'Yahavi.AI' },
  },
};
...
function parseResponse(rawText) {
  ...
  const grab = (r) => { const m = text.match(r); return m ? m[1].trim() : ''; };
...
router.post('/chat', async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, data: parseResponse(rawText) });
  } catch (err) {
